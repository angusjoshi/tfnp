/-
Vendored from https://github.com/math-xmum/Brouwer (commit unspecified)
Copyright (c) 2025 Math_XMUM. Released under the MIT License (see ../../LICENSE-Brouwer).

Status: minor mathlib API porting required for compatibility with current
mathlib (this project tracks `6cf3ab1c…`; upstream targets an older pin).
...
instance TT.IST : IsStrictTotalOrder (TT n l) (TT.Ilt i) where
  trichotomous := by
    intro a b h1 h2
    unfold TT.Ilt at h1 h2
    have heq : toLex (a i, a) = toLex (b i, b) := by
      rcases lt_trichotomy (toLex (a i, a)) (toLex (b i, b)) with h | h | h
      · exact absurd h h1
      · exact h
      · exact absurd h h2
    have hpair : (a i, a) = (b i, b) := heq
...
(h2 : σ.Nonempty):
  ...
  have h_not_dominant : ¬ TT.ILO.isDominant σ C := by
    intro hd
    obtain ⟨i, hi, hM_le⟩ := hd M
    obtain ⟨x, hx, hxlt⟩ := h_contradiction i hi
    letI : LinearOrder (TT n l) := IndexedLOrder.IST i
    exact absurd (hM_le x hx) (not_le.mpr hxlt)
      ...
      have h_bound_int : ∀ z ∈ σ, (z i : ℤ) - (m' i : ℤ) < C.card := by
        ...
        · have : (z i : ℕ) - m i < C.card := this
          ...
          have h_le : m i ≤ (z i : ℕ) := by
            ...
            apply Finset.mem_image_of_mem
            exact hz
          have : ((z i : ℕ) : ℤ) - (m i : ℤ) = (((z i : ℕ) - m i : ℕ) : ℤ) :=
            (Int.natCast_sub h_le).symm
          omega
  ...
  have imgs_nonempty : imgs.Nonempty := by
    ...
    have preimages_all_finite : ∀ a ∈ s, Set.Finite (e ⁻¹' {a}) := by
      ...
      have a_in_imgs : a ∈ imgs := by
        simp only [imgs, Finset.mem_filter]
        exact ⟨ha, hnf⟩
